...
- Full-Chain Acceptance Process: the sole `bug-fix` chain-continuity acceptance authority. Materialize `.governance/tests/test-bug-fix-contract.py` from immutable `062a6be3f0697c94fc42f4cc0d832129c128d503` to a path outside the repository; verify its raw-file SHA-256 equals `5678b16e2f9fa21cfc884c7af637a3cddbac163d039a4bb61068c96101def94a`; make that file read-only. Materialize the frozen behavioral oracle from immutable `09aeae77ec63cb9562080bcf4aa8df3635d87f49`; verify its raw-file SHA-256 equals `1980420dc5a0648cb1a7fc7ef65ab9ddacd062656b8abc34dc12fbac76b356ea`; make that file read-only. Run `python3 -I <full-chain-acceptance-process> --repo . --pre-ref <faulty-baseline-ref> --frozen-oracle-sha256 <sha256> --frozen-oracle-process <frozen-behavioral-oracle> --attempt <n>`. Attempt 2 or later must repeat `--previous-evidence-record <path>` once for every prior canonical v4 evidence record in attempt order. The Full-Chain Acceptance Process must invoke the frozen behavioral oracle with `python3 -I`. The worktree verifier is not acceptance authority unless its bytes are independently proven identical to the pinned Full-Chain Acceptance Process.
...
- The Full-Chain Acceptance Process is `.governance/tests/test-bug-fix-contract.py` at immutable `062a6be3f0697c94fc42f4cc0d832129c128d503`, raw-file SHA-256 `5678b16e2f9fa21cfc884c7af637a3cddbac163d039a4bb61068c96101def94a`. It is authoritative only for complete predecessor-chain continuity and delegates the behavioral verdict to the frozen targeted oracle.
- Materialize both processes outside the repository, independently SHA-verify each against its pinned identity, make both read-only before execution, and launch each verifier with isolated Python mode (`python3 -I`). Do not execute historical verifier content in-process.
...
- `BUGFIX-PYTHON-ISOLATION`: Before declaring the successor authority valid, launch both the Full-Chain Acceptance Process and its frozen behavioral-oracle child from an environment containing attacker-controlled shadow modules such as `argparse.py` and `sitecustomize.py`; prove those modules are not executed and cannot forge or alter acceptance evidence.
