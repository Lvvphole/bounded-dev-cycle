...
- For changes to the `bug-fix` route or its verification contract, run `python .governance/tests/test-bug-fix-contract.py --repo . --pre-ref <faulty-baseline-ref> --attempt <n>`; for attempt 2 or later, also pass `--previous-evidence-record <path>` for the immediately preceding canonical JSON evidence record.
